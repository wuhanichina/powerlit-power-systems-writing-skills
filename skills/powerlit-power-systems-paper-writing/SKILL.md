...
3. 🔴 CHECKPOINT: if the idea, model, or evidence package has not passed prewriting review, use `powerlit-power-systems-prewriting-review` first. Do not turn an unsupported paper into fluent manuscript prose, and do not polish a NO-GO idea past this gate.
4. Load `references/pre-drafting-confirmation.md` before full-paper drafting, major rewriting, title design, abstract, introduction, contribution, or venue-positioning work. Search the supplied project files first, then use PowerLit or literature retrieval when available to help judge whether the pain point is real, whether the innovation answers it, which innovation level is primary, how each candidate should be used in the manuscript, how the theoretical value positioning and engineering value positioning sit in the nearest literature family, and how the title should be positioned. Before drafting, return a short confirmation brief that lists the real industry or engineering pain point, file-search-confirmed innovation points, innovation level ladder, evidence maturity and manuscript use, PowerLit-backed theoretical and engineering value positioning, technical-level research significance, and feasible paper titles. The brief must use a professional problem statement rather than internal project names, must not reduce value to metric-level evidence, and it must avoid binary "supports/does not support" wording as the main innovation-mining language. 🔴 CHECKPOINT · 🛑 STOP: ask the user to confirm the engineering background, pain-point/innovation-level/value-positioning/title choice, and do not continue into full manuscript prose in the same response, unless the user has already confirmed them or explicitly asked you to proceed with your best choice.
...
14. Before finalizing, run the delivery gates in four groups. Do not skip a group to fit length:
   ...
   - **Structural (always):** the manuscript-section quality gate (including the spine-consistency check across title/abstract/introduction/results/conclusion) in `references/manuscript-section-quality.md` and the mandatory reader-experience pass in `references/reader-experience-pass.md`.
   - **Prose core passes (always):** the remaining core passes in `references/prose-quality-gates.md` — reader-burden cut, rhythm pass, lexicon pass, boundary-posture pass, non-binary manuscript framing pass, physical-story pass, and working-language firewall. Do not re-enumerate them elsewhere; this file is their single source.
...
For full-paper drafting, major rewriting, title design, abstract, introduction, contribution, or venue-positioning tasks where the pain point, innovation, value positioning, technical significance, or title is not already confirmed, return `写作前确认` first: real industry or engineering pain point, professional problem statement without internal project names, file-search-confirmed innovation points mapped to that pain point, innovation level ladder, evidence maturity and manuscript use, PowerLit-backed theoretical value positioning, engineering value positioning, metric-level evidence as subordinate demonstration, technical-level research significance, literature-near novelty risk or fallback status, feasible title candidates, and the specific confirmation needed from the user. Do not continue into full manuscript prose in that same response unless the user explicitly authorizes best-judgment drafting.
