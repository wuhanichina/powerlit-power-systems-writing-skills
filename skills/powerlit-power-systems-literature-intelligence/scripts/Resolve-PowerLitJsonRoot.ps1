...
foreach ($candidate in $candidates) {
    ...
    if (Test-Path -LiteralPath $candidate -PathType Container) {
        [pscustomobject]@{
            ...
            source = if ($candidate -eq $PowerLitJsonRoot) { "parameter" } elseif ($candidate -eq $env:POWERLIT_JSON_ROOT) { "POWERLIT_JSON_ROOT" } elseif ($candidate -eq $env:POWERLIT_LITERATURE_JSON) { "POWERLIT_LITERATURE_JSON" } else { "default" }
